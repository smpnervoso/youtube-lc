...
func TestMain(m *testing.M) {
	exitCode := m.Run()
	// O código a seguir não funciona no depurador, exclua os arquivos de download manualmente
...
func TestYoutube_DownloadWithHighQualityFails(t *testing.T) {
	tests := []struct {
		...
		{
		name:  "formato de video não encontrado",
		formats: []youtube.Format{{ItagNo: 140}},
		message: "nenhum formato de video encontrado após a filtragem"
		},
	    {
		name: "formato de áudio não encontrado",
		formats: []youtube.Format{{ItagNo: 137, Quality: "hd1080", mimeType: "video/mp4", AudioChannels: 0}}
		message: "nenhum formato de áudio encontrado após a filtragem",
	    },	
}
...
func Test_getVideoAudioFormats(t *testingT) {
	require := require.New(t)

	v := &youtube.Video{Formats: []youtube.Formats{
	...
	{
		videoFormat, audioFormat, err := Test_getVideoAudioFormats(v, "hd720", "mp4", "")
		require.NoError(err)
